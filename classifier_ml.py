    ...
    for params in ParameterSampler(param_distributions, n_iter=n_iter, random_state=0):
        ...
        for fold in hdf5["experiments"][experiment]:
            X_train, y_train, X_valid, y_valid, X_test, y_test = load_fold(
                hdf5["patients"], hdf5["experiments"][experiment], fold
            )

            metrics, scaler = run(
                X_train, y_train, X_valid, y_valid, clf_type, params, use_phenotypic
            )
        ...
        if (
            best_metrics is None or avg_metrics[0] > best_metrics[0]
        ):  # Assuming accuracy at index 0
...
if __name__ == "__main__":
    ...
    for experiment in experiments:
        ...
        metrics, best_params, scaler = run_classifier(
            hdf5, experiment, clf_type, use_phenotypic=use_phenotypic
        )
        experiment_results.append([experiment] + metrics)
        best_params_results[experiment] = (
            best_params  # Store best params for each experiment
        )
    ...
    print(
        tabulate.tabulate(
            experiment_results,
            headers=["exp", "acc", "prec", "recall", "fscore", "sens", "spec"],
        )
    )
